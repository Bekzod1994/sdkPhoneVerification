package uz.digid.myverdi.ui.views
...
class OverlayViewQrCode : View {
    init {
        ...
        val width = size.x
        val height = size.y
        init(R.color.design_default_color_on_primary, width / 10f, height / 8f)
    }
    private var transparentBackground: Paint? = null
    private var eraser: Paint? = null
    private var mFinderMaskPaint: Paint? = null
    ...
    constructor(context: Context?, @Nullable attrs: AttributeSet?, defStyleAttr: Int) : super(
     ...
     override fun onDraw(canvas: Canvas) {
        ...
        val rect = Rect(0, 0, measuredWidth, measuredHeight)
         ...
         transparentBackground?.let { canvas.drawPath(path, it) }
       /* canvas.drawArc(
            RectF(
                horizontalMargin,
                verticalMargin,
